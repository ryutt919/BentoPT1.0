...
def get_sequence_windows(keypoints, frame_numbers, labels_df, window_size=32, stride=16):
...
def prepare_dataset():
    # 기본 경로 설정
    base_dir = Path('mmaction2/data/kinetics400')
    npz_dir = base_dir / 'normalized' / 'bench_pressing'
    csv_dir = base_dir / 'labels' / 'bench_pressing'
    ...
    print(f"\nTotal sequences generated: {len(annotations)}")
    print(f"  - Labeled sequences: {len(labeled_data)}")
    print(f"  - Unlabeled sequences: {len(unlabeled_data)}")
    ...
    if unlabeled_data:
        unlabeled_dataset = {
            'split': {
                'unlabeled': [item['frame_dir'] for item in unlabeled_data]
            },
            'annotations': annotations  # 모든 데이터의 annotation 포함
        }
